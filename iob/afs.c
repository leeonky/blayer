...
static int parse_audio_frames(audio_frames *frames, const char *event_args) {
	int res = 0;
	if(9==sscanf(event_args, "rt:%d ch:%d fmt:%d buf:%d lay:%"PRIu64" align:%d cbuf:%d bits:%d size:%d", &frames->sample_rate, &frames->channels, &frames->format, &frames->buffer_samples, &frames->layout, &frames->align, &frames->cbuf_id, &frames->cbuf_bits, &frames->cbuf_size)) {
